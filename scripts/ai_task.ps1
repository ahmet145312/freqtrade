...
foreach ($inputItem in $inputs) {
  ...
  try {
    ...
    $localDir = Join-Path $runDir "local_files"
    ...
    $copiedFile = Join-Path $localDir $item.Name
    ...
    $notes.Add("Yerel dosya hazırlandı: $($item.FullName)") | Out-Null
  } catch {
    $errMsg = param(
  [Parameter(Position=0)]
  [string[]]$Task,
...
.Exception.Message
    $notes.Add("Yerel dosya hazırlık hatası: $inputItem - $errMsg") | Out-Null
  }
}
if ($detected.Count -eq 0) {
  $detected.Add("plain_task") | Out-Null
